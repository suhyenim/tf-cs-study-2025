...
### 한줄 답변
- 스레드 수는 CPU-bound는 코어 수, I/O-bound는 코어 × 대기비율로 결정하며, DB pool·대기 시간·TPS·메모리까지 고려해야 한다.
...
- 세그멘테이션은 논리적 단위라 보호/공유에 유리하지만 외부 단편화가 발생. 페이징은 고정 크기라 관리가 쉽고 외부 단편화가 없지만 내부 단편화가 발생할 수 있습니다.
...
### 한줄 답변
- 페이지에서 실제 주소를 가져오는 과정은 페이지 번호를 페이지 테이블로 조회해 프레임 번호를 얻고, 페이지 오프셋을 더해 물리 주소를 계산하는 방식이다.
...
- volatile 키워드: Java에서 변수의 메인 메모리 접근을 보장하고, 컴파일러/CPU 최적화로 인한 재정렬(reordering) 방지
- 특징
...
### 한줄 답변
- volatile은 변수의 가시성을 보장하고 명령 재정렬을 방지하지만, 복합 연산의 원자성은 보장하지 않는다.
...
- 동기(Synchronous): **호출한 함수**가 작업 완료까지 기다림 → 호출 스레드가 블록됨
- 비동기(Asynchronous): 호출 후 작업 완료를 기다리지 않고 다음 코드 실행 → 완료 시 콜백, Future, Promise 등으로 알림

- 블로킹(Blocking): **호출 스레드**가 I/O 또는 작업 완료 전까지 멈춤
- 논블로킹(Non-blocking): 호출 스레드가 즉시 반환 → 작업 진행은 백그라운드에서 수행

### 한줄 답변
- 동기/비동기는 작업 완료를 기다리는지 여부, 블로킹/논블로킹은 호출 스레드가 멈추는지 여부를 나타내며, 비동기 + 논블로킹이 가장 효율적이다.
...
- 동기이면서 논블로킹
  - 호출한 함수가 즉시 반환하지만, 결과는 기다리지 않음 → 이건 사실상 의미가 모호
  - 이유: 동기는 “작업 완료까지 기다린다”는 의미 → 즉시 반환하는 논블로킹과 모순

- 비동기이면서 블로킹
  - 호출 후 작업 완료를 기다리지 않는 비동기 개념과, 호출 스레드가 멈추는 블로킹 개념이 충돌

### 한줄 답변
- 동기+논블로킹과 비동기+블로킹은 원칙적으로 모순이지만, 비동기 내부 처리 후 블로킹 호출(Future.get 등)으로 제한적으로 발생할 수 있다.
