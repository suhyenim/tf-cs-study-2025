...
5. 사용자 프로그램에게 결과 반환
#### => 위 과정은 트랩(trap) 또는 소프트웨어 인터럽트를 통해 이루어진다
- 트랩 💡
  - **트랩 == 소프트웨어 인터럽트**
...
### 한줄 답변
- 인터럽트는 CPU가 하던 일을 잠깐 멈추고 긴급하거나 중요한 이벤트를 처리하도록 만드는 신호이다. HW 인터럽트는 외부의 물리적인 장치가 CPU에게 보내는 신호이고 SW 인터럽트는 내부 프로그램이 의도적으로 발생시키는 인터럽트이다.
...
### 한줄 답변
- 인터럽트는 우선순위가 있기 때문에 여러 개가 동시에 발생하면 높은 우선순위부터 처리한다. 필요하면 중첩하여 처리 중에도 더 중요한 인터럽트가 끼어들 수 있고, 하드웨어(APIC)가 큐잉과 마스킹을 조절해 충돌 없이 정교하게 처리한다.
...
- 초기화 하지 않은 전역 변수와 static 변수들은 BSS 영역(BSS Segment)에 저장된다.
...
### 한줄 답변
- 스택은 스레드 생성 시 최대 크기가 정해지고 그 안에서만 동적으로 변한다. 
...
### 한줄 답변
- Shared Memory는 mmap 기반 메모리 매핑 영역에 위치한다.
...
### 한줄 답변
- 스택은 생성 시 최대 크기가 정해지고 힙은 실행 중 확장되며 둘 다 사용자 설정으로 조정 가능하다.
...
- 장기 스케줄러는 어떤 작업을 메모리에 올릴지 결정한다(job → ready).
- 중기 스케줄러는 메모리 부족 시 프로세스를 swap-out하여 suspended 상태로 만든다.
...
- new, ready, running, waiting(blocked), terminated 상태가 있으며 I/O 요청 시 waiting으로 이동하고, CPU 스케줄링 대상이 되면 ready가 된다.
- running에서 타임 슬라이스가 끝나면 다시 ready로 전환된다.

### 한줄 답변
= 프로세스는 new → ready → running → waiting → terminated의 상태 변화를 가진다.
...
- 중기 스케줄러가 있는 경우 해당 프로세스는 suspended ready 또는 suspended waiting 상태로 바뀌어 디스크로 swap-out된다. 중기 스케줄러가 없다면 OOM 킬러가 프로세스를 종료할 수도 있다.

### 한줄 답변
- 메모리가 부족하면 프로세스는 suspended 상태로 이동하거나 OOM에 의해 종료된다.
...
- 현재 실행 중인 프로세스의 레지스터, PC, 스택 포인터 등 CPU 상태를 저장하고 다음 실행할 프로세스의 저장된 상태를 복구한다. 또한 MMU의 페이지 테이블도 교체된다(프로세스 간 전환 시).
...
### 한줄 답변
- 컨텍스트는 trap frame과 PCB 형태로 커널 스택에 저장된다.
...
- 타임 슬라이스 만료, I/O 요청으로 waiting 상태로 전환될 때, 더 높은 우선순위 프로세스 도착, sleep/wait 호출, 인터럽트 처리 시 발생한다.
...
- Priority 기반 Preemptive 스케줄링이 적합하다. 중요한 프로세스가 즉시 CPU를 받을 수 있기 때문이다.
...
- 하지만 배치 작업, 프린터 스풀러, 로그/백업 등 순차 처리가 중요한 환경에서는 구현이 단순하고 예측 가능해 여전히 유효하다.
...
### 한줄 답변
- 뮤텍스는 1개의 자원 보호용, 세마포어는 n개의 자원을 관리하는 카운터 기반 도구이다.
...
### 한줄 답변
- Deadlock은 서로의 자원을 기다리며 진행이 멈춘 상태이다.
...
- **Circular Wait(순환 대기)**: 자원 요청이 원형으로 이어져 각 프로세스가 다음 프로세스의 자원을 기다린다.

### 한줄 답변
- 상호배제·점유대기·비선점·순환대기의 네 조건이 모두 충족되어야 데드락이 발생한다.
...
### 한줄 답변
- Wait-Free는 전 스레드 보장, Lock-Free는 시스템 전체 진행만 보장한다.
...
- **링커**: 빌드 시 여러 오브젝트 파일을 결합하고, 심볼 해석·재배치·주소 할당을 수행해 실행 파일/공유 라이브러리를 만든다.
- **로더**: 실행 시 실행 파일을 메모리에 적재하고, 동적 라이브러리를 연결한 뒤 프로그램을 시작한다.
- 시점과 역할이 완전히 다르다.
...
### 한줄 답변
- JIT는 실행 중 코드 일부를 기계어로 변환해 속도를 높이는 방식이다.
...
- `fork()`로 부모 프로세스를 복제한 뒤 `exec()`를 호출하면 기존 주소 공간을 새로운 프로그램으로 덮어쓴다.
